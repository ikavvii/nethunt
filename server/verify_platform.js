  ...
  try {
    ...
    await fetch(`${BASE}/api/admin/config`, {
      ...
      body: JSON.stringify({
        ...
        event_end_date: '2026-09-19T10:00:00+05:30'
      })
    });

    const evStatusRes = await fetch(`${BASE}/api/events/status`).then(r => r.json());
    assert(evStatusRes.eventWindow === '12 Sep 2026 (09:00 AM) – 19 Sep 2026 (10:00 AM)', 'EVENT WINDOW: /api/events/status reports 12 Sep 2026 (09:00 AM) – 19 Sep 2026 (10:00 AM)');
    assert(evStatusRes.eventStartDate === '2026-09-12T09:00:00+05:30', 'EVENT WINDOW: Start date is 2026-09-12T09:00:00+05:30');
    assert(evStatusRes.eventEndDate === '2026-09-19T10:00:00+05:30', 'EVENT WINDOW: End date is 2026-09-19T10:00:00+05:30');
    ...
    await db.prepare("UPDATE config SET value = '2026-09-19T10:00:00+05:30' WHERE key = 'event_end_date'").run();
    await db.prepare("UPDATE config SET value = ? WHERE key = 'event_end_time'").run(String(new Date('2026-09-19T10:00:00+05:30').getTime()));
